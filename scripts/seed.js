...
/*
 * Cria dados de demonstracao para testar a agenda.
 *   npm run seed
...
for (const person of people) {
  ...
  const info = db
    .prepare('INSERT INTO users (name, email, password_hash, role, color, created_at) VALUES (?, ?, ?, ?, ?, ?)')
    .run(person.name, person.email, hash, person.role, person.color, iso(now));
...
for (const event of events) {
  const already = db.prepare('SELECT id FROM events WHERE title = ? AND start_at = ?').get(event.title, event.start);
  if (already) continue;
  ...
  const info = db
    .prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    .run(event.title, event.description || '', event.location || '', event.start, event.end, event.allDay || 0, event.color, author, author, iso(now), iso(now));

  db.prepare('INSERT INTO activity (event_id, user_id, action, detail, created_at) VALUES (?, ?, ?, ?, ?)')
    .run(info.lastInsertRowid, author, 'criou', event.title, iso(now));
}

const first = db.prepare('SELECT id FROM events ORDER BY id LIMIT 1').get();
if (first && !db.prepare('SELECT id FROM comments WHERE event_id = ?').get(first.id)) {
  db.prepare('INSERT INTO comments (event_id, user_id, body, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
    .run(first.id, ids['bruno@empresa.com'], 'Consigo participar, mas preciso sair 10 minutos antes.', iso(now), iso(now));
  db.prepare('INSERT INTO comments (event_id, user_id, body, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
    .run(first.id, ids['carla@empresa.com'], 'Vou levar os números do suporte para a pauta.', iso(now), iso(now));
}
